...
.Glossary entries
* *xref:backup-storage-location[backup storage location]*: Added glossary entry
* *xref:identity-provider[<term>]*: Updated glossary entry to use "IdP" as the acronym

.Style guidance
* *xref:users[Users]*: Added guidance on how to refer to animate and inanimate users

.Other updates
* Added a link to the online _IBM Style_ guide that Red{nbsp}Hat associates can use
...
_No glossary or style updates_
